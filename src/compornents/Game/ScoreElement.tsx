...
export default function ScoreElement(props: Props) {
    ...
    return (<g width={FieldSize} height={FieldSize}>
        ...
        <text x={(0 + 0.9) * (cellSize)} y={(3 - 0.2) * (cellSize)}
            textAnchor="middle" dominantBaseline="central" fill="#000000" fontSize={15}>▶︎</text>
        <text x={(0 + 0.5) * (cellSize)} y={(5 - 0.2) * (cellSize)}
            textAnchor="middle" dominantBaseline="central" fill="#000000" fontSize={15}>{props.field.calc_bottom() + ' hit'}</text>
